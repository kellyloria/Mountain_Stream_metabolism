...
data_dir <- "R:/Users/kloria/Documents/Stream_Metab_24/BWU_DO/RawDO_BWU"
...
rawdat1 <- rawdat %>%
  mutate(
    ...
    PCT_datetime = force_tz(PCT_datetime, tzone = "UTC"),
    
    # Convert from UTC to Pacific Time (automatically adjusts for daylight savings)
    PCT_datetime_adjusted = with_tz(PCT_datetime, tzone = "America/Los_Angeles")
  )

# Verify the output
rawdat2<- na.omit(rawdat1)

summary(rawdat$PCT_datetime_adjusted)
...
do.ts$site <- "BWU"
# write.csv(x = do.ts, file = "R:/Users/kloria/Documents/Stream_Metab_24/BWU_DO/raw_merged_dat/24_BWU_rawDO_record.csv", row.names = TRUE)
# saveRDS(do.ts, file = "R:/Users/kloria/Documents/Stream_Metab_24/BWU_DO/raw_merged_dat/24_BWU_rawDO_record.rds")
