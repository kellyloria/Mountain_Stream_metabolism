...
#"R:\Users\kloria\Documents\Stream_Metab_24\Core_sites\offset_DO_dat\saturation\24_BWU_DO_flag_sat.rds"

site <- "BWU"
dat <- readRDS("R:/Users/kloria/Documents/Stream_Metab_24/Core_sites/offset_DO_dat/light/24_BWU_DO_flag_sat_light.rds") %>%
  filter(maintenance_flag<1 & maintenance_flag<1 & datetime > as.POSIXct("2021-06-30 00:00:00") & datetime < as.POSIXct("2024-08-01 00:00:00") &  do.obs>3.85)
...
dat <- dat %>%
  ...
  summarize(
    ...
    .groups = "drop") %>%
  mutate(
    date_only = as.Date(rounded_datetime)
  ) %>%
  group_by(date_only) %>%
  # Filter for days with exactly 24 hourly observations
  filter(n() == 24) %>%
  ungroup() %>%
  select(-date_only)

dat$solar.time <- calc_solar_time(dat$rounded_datetime, -120.164335)

plot(dat$solar.time, dat$rounded_datetime)
...
# 
logK600 = c(8.6805*(dat3$discharge) + 2.4309)
...
bayes_specs_new$K600_lnQ_nodes_meanlog <- c(11.99, 5.55, 4.24, 3.27, 2.78, 2.54) # Example mean(log(K600)) from  data
...
# Run the model
##==============
dat_metab_BWU <- metab(bayes_specs_new, data = dat3)

# extract fit:
dat_fit_BWU <- get_fit(dat_metab_BWU)

## Visualize
DOplot <-plot_DO_preds((dat_metab_BWU))
DOplot
# 
ggsave(plot = DOplot, filename = paste("R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/Full_figure/DO_preds_BWU_bayesmodel_mK600_filter_hr_250107.png",sep=""),width=10,height=4,dpi=300)

metabplot<- plot_metab_preds(predict_metab(dat_metab_BWU))
# 
ggsave(plot = metabplot, filename = paste("R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/Full_figure/metabplot_BWU_bayesmodel_mK600_filter_hr_250107.png",sep=""),width=10,height=4,dpi=300)
...
binplot<- Binning(dat_fit_BWU, dat_metab_BWU)

ggsave(plot = binplot, filename = paste("R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/Full_figure/binplot_BWU_bayesmodel_mK600_filter_hr_25010.png",sep=""),width=5,height=4,dpi=300)


get_fit(dat_metab_BWU)$overall %>%
  dplyr::select(ends_with('Rhat')) # might be best rhat 

## optional save details
site <- "BWU"
rundate <- format(Sys.Date(), "%y%m%d")
runmeta_k600 <- "K600_m_prior_filter_hr"
runmeta_light <- "NLDAS"
...
writefiles(dat_fit_BWU, dat_metab_BWU)


getwd()


# saveRDS(dat_metab_BWU, file = "R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/Full_out/BWU_Summer_K600_measured_prior_noGPPprior_241225.rds")
...
# plot for k600
# R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/BWU_HighFlow_test_pool_K600_binned_NLDAS_241204datadaily.csv
BWU <- read.csv("R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/BWU_K600_est_prior_filter_NLDAS_daily250102.csv")
...
#
ggsave(plot = k_grid, filename = paste("R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/Full_out/ER_K600plot_BWU_K600_est_prior_filter_NLDAS_daily250102.png",sep=""),width=6,height=3,dpi=300)
...
ggsave(plot = k_e_grid, filename = paste("R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/Full_out/BWU_K600_conditions_prior_filter_NLDAS_daily250102.png",sep=""),width=4,height=12,dpi=300)
