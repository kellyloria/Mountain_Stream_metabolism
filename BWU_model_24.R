...
site <- "BWU"
dat <- readRDS("R:/Users/kloria/Documents/Stream_Metab_24/Core_sites/offset_DO_dat/light/24_BWU_DO_flag_sat_light.rds") %>%
  filter(maintenance_flag<1 &
           fouling_flag<1 &
    datetime > as.POSIXct("2021-06-30 00:00:00") &
           datetime < as.POSIXct("2023-10-25 00:00:00")) # & fouling_flag<1 &  datetime > as.POSIXct("2021-04-29 11:15:39"))
summary(dat)
...
Summer_test <- subset(dat, solar.time > "2022-05-05 00:00:00" & solar.time < "2023-01-20 20:00:00")
...
bayes_specs_new$stan_control <- list(adapt_delta = 0.95, max_treedepth = 15)
# defaults at adapt_delta = 0.95, max_treedepth = 10
# 
...
# 
logK600 = c(8.6805*(dat3$discharge) + 2.4309) #y = mx+b from measured flow and K
...
bayes_specs_new$K600_lnQ_nodes_centers <- seq(logQ_range[1], logQ_range[2], length.out = 6)
bayes_specs_new$K600_lnQ_nodes_meanlog <- c(9.79, # mean logK600
                                            5.26, # median logK600
                                            4.24, # measured
                                            3.27, # measured
                                            2.78, # measured
...
# Run the model
##==============
dat_metab_BWUp <- metab(bayes_specs_new, data = dat3)

# extract fit:
dat_fit_BWUp <- get_fit(dat_metab_BWUp)

## Visualize
DOplot <-plot_DO_preds((dat_metab_BWUp))
DOplot
# 
ggsave(plot = DOplot, filename = paste("R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/Full_figure/DO_preds_BWUp_bayesmodel_measuredK600_filter.png",sep=""),width=10,height=4,dpi=300)

metabplot<- plot_metab_preds(predict_metab(dat_metab_BWUp))
# 
ggsave(plot = metabplot, filename = paste("R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/Full_figure/metabplot_BWUp_bayesmodel_measuredK600_filter.png",sep=""),width=10,height=4,dpi=300)
...
binplot<- Binning(dat_fit_BWUp, dat_metab_BWUp)

ggsave(plot = binplot, filename = paste("R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/Full_figure/binplot_BWUp_bayesmodel_measuredK600_filter.png",sep=""),width=5,height=4,dpi=300)

# ggsave(plot = binplot, filename = paste("R:/Users/kloria/Documents/2023_StreamMetab/figures/binplot_BWL_bayesmodel_BWL2.png",sep=""),width=5,height=4,dpi=300)


get_fit(dat_metab_BWUp)$overall %>%
...
writefiles(dat_fit_BWUp, dat_metab_BWUp)


getwd()


# saveRDS(dat_metab_BWUp, file = "R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/Full_out/BWUp_Summer_K600_measured_prior_noGPPprior_241210.rds")
...
BWUp <- read.csv("R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/BWU_K600_measured_prior_filter_NLDAS_daily241216.csv")

BWUp$date <- as.Date(BWUp$date, origin="2021-01-01")
BWUp$site <- "BWU"
BWUp$shore <- "west"

Odd_plot <- ggplot(BWUp, aes(x = K600_daily_mean, color = shore, fill = shore)) +
  ...
  geom_vline(data = BWUp, aes(xintercept = mean(na.omit(K600_daily_mean))), linetype = "dashed") 

Gplot_sp <- ggplot(BWUp, aes(x = K600_daily_mean, y = (ER_mean*-1))) +  
  geom_point(shape= 17, col = alpha(c("#3283a8"),0.75)) +
  geom_smooth(method ="lm", se=F)+  facet_grid(.~site)+
  theme_bw()


GB_lm <- lm(K600_daily_mean~(ER_mean*-1), data=BWUp)
summary(GB_lm)

# Extract R-squared value
r_gsquared <- summary(GB_lm)$r.squared
GB_plot <- Gplot_sp + 
  geom_text(aes(x = mean((BWUp$K600_daily_mean), na.rm=T), y = max((BWUp$ER_mean*-1), na.rm=T), 
...
#
ggsave(plot = k_grid, filename = paste("R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/Full_out/ER_K600plot_BWU_K600_measured_prior_filter_NLDAS_daily241216.png",sep=""),width=6,height=3,dpi=300)
...
Gplot_DOsat <- ggplot(BWUp2, aes(y = K600_daily_mean, x = DO.sat)) +  
...
Gplot_temp <- ggplot(BWUp2, aes(y = K600_daily_mean, x = temp.water)) +  
  geom_point(shape= 17, col = alpha(c("#3283a8"),0.75)) +
  #geom_smooth(method ="lm", se=F)+ 
  facet_grid(.~site)+
  theme_bw()

Gplot_Q <- ggplot(BWUp2, aes(y = K600_daily_mean, x = discharge)) +  
  geom_point(shape= 17, col = alpha(c("#3283a8"),0.75)) +
  #geom_smooth(method ="lm", se=F)+  
  facet_grid(.~site)+
  theme_bw()
...
GB_lmq <- lm(K600_daily_mean~discharge, data=BWUp2)
summary(GB_lmq)

# Extract R-squared value
r_gsquaredq <- summary(GB_lmq)$r.squared
Gplot_Q2 <- Gplot_Q + 
  geom_text(aes(y = mean((BWUp2$K600_daily_mean), na.rm=T), x = max((BWUp2$discharge), na.rm=T), 
                label = paste("R2=", round(r_gsquaredq, 3))),
            hjust = 1, vjust = 0, size = 3, col = "blue",
            parse = T, check_overlap = T, na.rm = T)

Gplot_z <- ggplot(BWUp2, aes(y = K600_daily_mean, x = depth)) +  
  geom_point(shape= 17, col = alpha(c("#3283a8"),0.75)) +
  #geom_smooth(method ="lm", se=F)+  
  facet_grid(.~site)+
  theme_bw()

GB_lmz <- lm(K600_daily_mean~depth, data=BWUp2)
summary(GB_lmz)
r_gsquaredz <- summary(GB_lmz)$r.squared

Gplot_z2 <- Gplot_z + 
  geom_text(aes(y = mean((BWUp2$K600_daily_mean), na.rm=T), x = max((BWUp2$depth), na.rm=T), 
...
ggsave(plot = k_e_grid, filename = paste("R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/Full_out/BWU_K600_conditions_prior_filter_NLDAS_daily241216.png",sep=""),width=4,height=12,dpi=300)
